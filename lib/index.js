...
function toSpans(positions, offset) {
  ...
  for (const position of positions) {
    ...
    if (last !== void 0 && last.end === position + offset) last.end = position + offset + 1;
...
function scoreEntry(queryLower, piecesLower, name2, nameLower, path, pathLower, scratch) {
  ...
  for (const piece of piecesLower) {
    const nameScore = isSubsequence(piece, nameLower) ? scoreFuzzy(piece, name2, nameLower, scores, matches, positions) : 0;
    if (nameScore !== 0) {
      total += nameScore;
    ...
    if (!isSubsequence(piece, pathLower)) return void 0;
...
function queryIndex(entries, query, maxMatches) {
  ...
  for (const entry of entries) {
    ...
    const scoredEntry = scoreEntry(trimmed, pieces, name2, nameLower, entry.path, entry.pathLower, scratch);
